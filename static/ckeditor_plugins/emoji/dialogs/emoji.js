CKEDITOR.dialog.add( 'emojiDialog', function( editor ) {
	...
	var buildHtml = function(group) {
		...
		var html = [
			'<div style="max-height:300px;overflow-y:scroll;"><style type="text/css">emoji:hover {border:1pt solid;border-color: green;background-color:#e6ffff}</style>' +
		...
		for (var shortcode in config.emoji.emojis[group]) {
			var obj = config.emoji.emojis[group][shortcode];
					list[obj] = shortcode;
		...
		for (var shortcode in list) {
			...
			html.push(
				...
				'<img class="emoji" cke_src="'+ CKEDITOR.tools.htmlEncode(urlimage) + '" src="'+ urlimage + '" href="javascript:void(0)" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );"/>', '</td>'
			);
		...
		contents: [
			{
				...
				id: 'tab-red',
				label: editor.config.emoji.tabs.red,
				elements: [
					emojis('red')
				]
			}, {
				id: 'tab-milkbottle',
				label: editor.config.emoji.tabs.milkbottle,
				elements: [
					emojis('milkbottle')
				]
			}, {
